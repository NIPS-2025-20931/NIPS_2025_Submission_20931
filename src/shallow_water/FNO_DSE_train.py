...
def samplepoints(args, device, points, radius):
    ...
    points, reordered_index, npts = MultiResGraph(points=points.numpy(), index=index_arr, levels=3, pc2gfactor=4.0, g2gfactor=4.0, method='random')

    points = torch.from_numpy(points)

    g_points = points[None,...].to(device)
    g_points = g_points.repeat(args.batch_size, 1, 1)
    transform = VFT3D(g_points[:,:,0], g_points[:,:,1], g_points[:,:,2], args.num_modes)

    return points, reordered_index, transform


@hydra.main(version_base=None, config_path="conf", config_name="FNO_DSE_train")
def main(args : DictConfig):
    ...
    reordered_xyz_coords, reordered_index, all_transform = samplepoints(args, device, torch.from_numpy(xyz_coords.astype(np.float32)), radius)

    ####################################################################################################
    model = FNO_DSE_SR_3D(input_dim_edge=input_dim_edge, input_dim_node=input_dim_node, num_modes=args.num_modes, hidden_dim=args.latent_feats, output_dim_node=out_features)
    ...
    for epoch in tqdm(range(args.start_epoch, args.epochs)):
        ...
        for tidx in range(math.ceil(len(data_indices)/args.batch_size)):
            ...
            # ===================forward=====================
            model_output = model(lf_feats, all_transform)
