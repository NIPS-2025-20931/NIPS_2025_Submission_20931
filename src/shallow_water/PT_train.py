...
@hydra.main(version_base=None, config_path="conf", config_name="PT_train")
def main(args : DictConfig):
    ...
    pos = batch_pos.cpu().numpy()

    num_samples = batch_pos.shape[1]

    ####################################################################################################
    model = PT_SR_3D(in_features=in_features, hidden_layers=args.hidden_layers, hidden_features=args.hidden_features, enc_layers=args.enc_layers, \
                    knn_neighbors=args.knn_neighbors, latent_features=args.latent_feats, feat_dim=4, transformer_dim=args.transformer_dim)
    ...
    for epoch in tqdm(range(args.start_epoch, args.epochs)):
        ...
        for tidx in range(math.ceil(len(data_indices)/args.batch_size)):
            ...
            # ===================forward=====================
            model_output = model(batch_pos, torch.cat([batch_pos, lf_feats], dim=-1))
