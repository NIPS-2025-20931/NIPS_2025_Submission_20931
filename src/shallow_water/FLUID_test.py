...
@hydra.main(version_base=None, config_path="conf", config_name="FLUID_test")
def main(args : DictConfig):
    ...
    grid_sample_points, reordered_index, grid_pc2g, grid_g2g, grid_transform, grid_g2pc = samplepoints2graphs(args, device, torch.from_numpy(xyz_coords.astype(np.float32)), radius)
    ...
    model = FLUID_3D(input_dim_edge=input_dim_edge, input_dim_node=input_dim_node, num_modes=args.num_modes, hidden_dim=args.latent_feats, output_dim_node=out_features)
