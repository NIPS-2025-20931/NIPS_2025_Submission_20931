...
@hydra.main(version_base=None, config_path="conf", config_name="MGN_test")
def main(args : DictConfig):
    ...
    pos = torch.from_numpy(xyz_coords.astype(np.float32))
    batch_pos = pos[None,...]
    batch_pos = batch_pos.to(device)

    edge_index = torch.from_numpy(edge_index.astype(np.int64)).t()
    ...
    ####################################################################################################
    model = MGN_SR_3D(in_features=in_features, hidden_layers=args.hidden_layers, hidden_features=args.hidden_features, out_features=out_features, \
                        node_dim=1, edge_dim=4, g_features=args.g_feats, latent_features=args.latent_feats, enc_layers=args.enc_layers, proc_layers=args.proc_layers, \
                        dec_layers=args.dec_layers, msg_passing_steps=args.msg_passing_steps, pos_freq_const=10, pos_freq_num=pos_freq_num, \
                        enc_lastact=args.enc_last_act, enc_bias=args.enc_bias, proc_lastact=args.proc_last_act, proc_bias=args.proc_bias, \
                        dec_lastact=args.dec_last_act, dec_bias=args.dec_bias, graph_nodes=num_samples, pooling=args.latent_fuse)
    ...
    with torch.no_grad():
        ...
        for i in range(dataidxrange[args.dataset]['test'][1]-dataidxrange[args.dataset]['test'][0]):
            ...
            for j in range(12):
                ...
                # ===================forward=====================
                model_output = model(batch_pos, edge_index, lf_feats, edge_features)
                ...
                # print('DataIndex = {0} Timestep = {1}, PSNR = {2:.4f}'.format(i, j, psnr))
                curpsnr += psnr
                ...
                # plt.scatter(expanded_phi.flatten(), expanded_theta.flatten(), c=model_output, vmin=0.0, vmax=1.0, cmap='RdBu')
                # plt.tight_layout()
                # plt.axis('off')
                # plt.savefig(args.dir_outputs + args.network_str + "_{0}_{1}.jpg".format(i, j), bbox_inches='tight', pad_inches=0)
                # plt.clf()
