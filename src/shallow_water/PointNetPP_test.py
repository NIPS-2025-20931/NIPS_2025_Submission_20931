...
@hydra.main(version_base=None, config_path="conf", config_name="PointNetPP_test")
def main(args : DictConfig):
    ...
    ####################################################################################################
    model = PointNetPP_SR_3D(in_features=in_features, hidden_layers=args.hidden_layers, hidden_features=args.hidden_features, out_features=out_features, \
    ...
    with torch.no_grad():
        ...
        for i in range(dataidxrange[args.dataset]['test'][1]-dataidxrange[args.dataset]['test'][0]):
            ...
            for j in range(12):
                ...
                # ===================forward=====================
                model_output = model(batch_pos, batch_pos.permute(0, 2, 1), lf_feats.permute(0, 2, 1))
