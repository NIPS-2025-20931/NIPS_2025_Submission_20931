...
class GINO(BaseModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.fno_in_channels = in_gno_out_channels*2
    ...
    def forward(self, lowres_geom, highres_geom, latent_queries, output_queries, x=None, latent_features=None, ada_in=None, **kwargs):
        ...
        lowres_geom = lowres_geom.squeeze(0)
        highres_geom = highres_geom.squeeze(0)
        latent_queries = latent_queries.squeeze(0)

        # Pass through input GNOBlock 
        in_p = self.gno_in(y=lowres_geom,
                           x=latent_queries.view((-1, latent_queries.shape[-1])),
                           f_y=x)

        
        # GNOBlock for highres
        highres_p = self.gno_highres(y=highres_geom,
                                    x=latent_queries.view((-1, latent_queries.shape[-1])))
        if batch_size > 1:
            highres_p = highres_p.repeat(batch_size, 1).reshape(*in_p.shape)
        else:
            highres_p = highres_p.reshape(*in_p.shape)

        in_p = torch.cat([in_p, highres_p], dim=-1)
...
class GINO_SR(nn.Module):
    def __init__(self, in_channels, out_channels, gno_transform_type, lifting_channels, gno_radius=0.1, projection_channels=256, gno_coord_dim=2):
        super().__init__()
        # "gno_transform_type": ["linear", "nonlinear_kernelonly", "nonlinear"]
        # "fno_norm": [None, "ada_in"]
        fno_n_modes = (16, 16, 16)
        self.gino = GINO(
            in_channels=in_channels,
            out_channels=out_channels,
            latent_feature_channels=None,
            gno_radius=gno_radius,# make this large to ensure neighborhoods fit
    ...
    def forward(self, lowres_geom, highres_geom, latent_queries, output_queries, x=None, latent_features=None, ada_in=None):
        out = self.gino(x=x,
                lowres_geom=lowres_geom,
                highres_geom=highres_geom,
