...
@hydra.main(version_base=None, config_path="conf", config_name="GCN_test")
def main(args : DictConfig):
    ...
    out_features = 1
    pos_freq_num = 30
    pos_features = in_features*2*pos_freq_num
    # pos_features = args.pos_feats
    ...
    model = GCN_SR(in_features=in_features, hidden_layers=args.hidden_layers, hidden_features=args.hidden_features, out_features=out_features, g_channel=1, \
                       g_layers=args.msg_passing_steps, g_features=args.g_feats, graph_nodes=selected_samples.shape[0], pos_freq_const=10, pos_freq_num=pos_freq_num, pooling=args.latent_fuse)
