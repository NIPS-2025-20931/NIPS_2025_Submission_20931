...
@hydra.main(version_base=None, config_path="conf", config_name="GINO_test")
def main(args : DictConfig):
    ...
    data_indices = np.arange(0, dataidxrange[args.dataset]['test'][1]*80, dtype=int)
    ...
    with torch.no_grad():
        ...
        for i in range(dataidxrange[args.dataset]['test'][1]-dataidxrange[args.dataset]['test'][0]):
            ...
            for j in range(80):
                ...
                # ===================forward=====================
                model_output = model(lowres_geom=batch_pos, 
                                        highres_geom=batch_pos, 
                                        latent_queries=latent_geom, 
                                        output_queries=batch_pos, 
                                        x=lf_feats, 
